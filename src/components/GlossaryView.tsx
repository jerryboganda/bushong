...
export const GlossaryView: React.FC<GlossaryViewProps> = ({ initialQuery = '' }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto px-3.5 xs:px-4 py-6 sm:py-8">
      {/* Header */}
      <div className="bg-gradient-to-r from-cyan-950/60 via-slate-900 to-teal-950/60 border border-cyan-500/30 rounded-2xl p-4 xs:p-6 sm:p-8 mb-6 sm:mb-8 shadow-xl">
        ...
        <div className="relative mt-6">
          ...
          <input
            ...
            className="w-full bg-slate-950 border border-slate-700 rounded-xl pl-10 pr-10 py-2.5 min-h-[44px] text-xs sm:text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-cyan-500 transition-all"
          />
          {searchTerm && (
            <button
              onClick={() => setSearchTerm('')}
              className="absolute right-2.5 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center text-slate-400 hover:text-white rounded-lg"
            >
              <X className="w-4 h-4" />
            </button>
          )}
        ...
        <div className="flex flex-wrap gap-1.5 mt-4 pt-3 border-t border-slate-800/80">
          {categories.map((cat) => (
            <button
              ...
              className={`px-3 py-1.5 min-h-[32px] rounded-lg text-xs font-medium transition-all ${
      ...
      <div className="flex flex-wrap items-center justify-center gap-1 sm:gap-1.5 mb-6 p-1.5 xs:p-2 bg-slate-900 border border-slate-800 rounded-xl">
        {alphabet.map((letter) => (
          <button
            key={letter}
            onClick={() => setSelectedLetter(letter)}
            className={`w-7 h-7 xs:w-8 xs:h-8 min-h-[28px] xs:min-h-[32px] rounded-lg text-[11px] xs:text-xs font-bold font-mono transition-all flex items-center justify-center ${
      ...
      <div className="space-y-4">
        {filteredGlossary.length === 0 ? (
          <div className="text-center py-12 bg-slate-900 border border-slate-800 rounded-xl p-4">
